...
{
    request->reqHeaders[request->reqHeadersNum].key = (char*)key; //保存外部堆内存的地址
...
char* httpRequestGetHeader(struct HttpRequest* request, const char* key)
{
    if(request != NULL)
    {
        ...
        {
            //由于保存的是地址值，所以不能这样作比较request->reqHeaders[i].key == key
...
char* splitRequestLine(const char* start, const char* end, const char* sub, char** ptr)
{
    /**
     * Q:函数最后一个参数为什么要传二级指针？
     * A:如果传入的是一级指针，那么会对该参数进行拷贝，也就是说传入的是地址的拷贝，
     * 只是该拷贝的对象与传入的对象指向同一个地址，但是在函数体内对该拷贝对象的操作
     * 并不会影响函数体外的原对象，如果将另外一个地址赋值给拷贝对象，原对象还是指向
     * 原来的地址；如果想要变更原对象保存的地址值，就需要传入二级指针，原对象是指针
     * 那么二级指针就是原对象这个指针的地址，只有这样，才能在函数体内部对原对象指向
     * 的地址进行修改，在C++中想要达到同样的效果可以传递一个引用。
     * 
     * 总结：如果想要在一个函数里面给外部的一个一级指针分配一块内存，就需要把外部的一级
     * 指针的地址传递给这个函数，一级指针的地址就是二级指针。
     */
    char* space = end;
    if(sub != NULL)
    {
    ...
    *ptr = tmp;
    ...
    return space + 1; //通过返回值知道下一个要处理的指针位置
}
...
bool parseHttpRequestLine(struct HttpRequest* request, struct Buffer* readBuf)
{
    ...
    if (lineSize > 0)
    {
        ...
        request->method = (char*)malloc(methodSize + 1); // +1是为了存放最后的\0
        strcpy(request->method, start, methodSize);
        request->method[methodSize] = '\0'; //数组下标不能为 methodSize+1
        ...
        strcpy(request->url, start, urlSize);
        request->method[urlSize] = '\0'; 

        //http版本
        start = space + 1;
        //只剩下最后一部分没有处理，不用再像之前那样操作了
        // char* space = memmem(start, end - start, " ", 1);
        // assert(space != NULL);
        // int urlSize = space - start;
        request->url = (char*)malloc(end - start + 1); 
        strcpy(request->url, start, end - start);
        request->method[end - start] = '\0'; 
#endif

        //为解析请求头做准备
        readBuf->readPos += lineSize; //跳到末尾
...
bool parseHttpRequestHeader(struct HttpRequest* request, struct Buffer* readBuf)
{
    ...
    if(end != NULL)
    {
        ...
        if (middle != NULL)
        {
            char* key = (char*)malloc(middle - start + 1);//+1是为了存放最后的\0
            strncpy(key, start, middle - start);
            key[middle-start] = '\0';

            char* value = (char*)malloc(end - middle - 2 + 1);//+1是为了存放最后的\0
...
bool processHttpRequest(struct HttpRequest* request, struct HttpResponse* response)
{
    ...
    if (ret == -1)
    {
        ...
        // 函数指针指向的这个函数是在 httpResponsePrepareMsg函数最后被调用的
...
void sendDir(const char* dirName, struct Buffer* sendBuf, int cfd)
{
    ...
    for (int i = 0; i < num; ++i)
    {
        ...
        bufferSendData(sendBuf, cfd); // 放一点发一点，避免内存不够用
#endif
