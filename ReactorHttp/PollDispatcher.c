...
static int pollAdd(struct Channel* channel, struct EventLoop* evLoop)
{
    ...
    for (; i < Max; ++i)
    {
        if (data->fds[i] = -1)//如果该位置没有文件描述符，就可以添加
        {
...
{
    struct PollData* data = (struct PollData*)evLoop->dispatcherData;
    int count = pool(data->fds, data->maxfd+1, timeout*1000);
    if (count == -1)
    {
        perror("poll");
        exit(0);
    ...
    for(int i = 0; i <= data->maxfd; ++i)
    {
        ...
        {
            eventActive(evLoop, data->fds[i].fd, ReadEvent);
        }
        if(data->fds[i].revents & POLLOUT)
        {
            eventActive(evLoop, data->fds[i].fd, WriteEvent);
        }
    }
    return 0;
}
